...
export default function Page() {
  ...
  const [selectedWork, setSelectedWork] = useState<{ title: string; file: string } | null>(null)
  const previousWorks = [{ title: 'تأثير التكنولوجيا على الخدمات التعليمية', file: '/files/technology-education.pdf' }, { title: 'حماية البيئة في ظل رؤية المملكة 2030', file: '/files/environment-law.pdf' }, { title: 'تطوير الصناعات المحلية والخدمات اللوجستية', file: '/files/local-industries.pdf' }, { title: 'المبتدأ والخبر في القرآن الكريم', file: '/files/arabic-language.pdf' }, { title: 'مشروع إقامة ذكية SmartStay', file: '/files/smartstay.pdf' }, { title: 'الفروق الفقهية في الأحوال الشخصية', file: '/files/fiqh-differences.pdf' }]
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="portfolio" className="portfolio-section container"><div className="section-heading"><div><span className="section-kicker">أعمالنا السابقة</span><h2>نماذج من <em>أعمالنا</em></h2></div><span className="portfolio-hint">معاينة فقط داخل الموقع</span></div><div className="portfolio-grid">{previousWorks.map((work) => <button className="portfolio-work-card" key={work.file} onClick={() => setSelectedWork(work)}><span className="portfolio-file-icon"><FileText size={28} /><small>PDF</small></span><span className="portfolio-work-info"><strong>{work.title}</strong><small>اضغط للمعاينة</small></span><ChevronLeft size={18} /></button>)}</div></section>

      {selectedWork && <div className="pdf-modal-backdrop" role="presentation" onClick={() => setSelectedWork(null)}><section className="pdf-modal" role="dialog" aria-modal="true" aria-labelledby="pdf-title" onClick={(event) => event.stopPropagation()}><div className="pdf-modal-header"><h2 id="pdf-title">{selectedWork.title}</h2><button onClick={() => setSelectedWork(null)} aria-label="إغلاق المعاينة"><X size={20} /></button></div><div className="pdf-viewer"><iframe src={selectedWork.file} title={`معاينة ${selectedWork.title}`} /><div className="pdf-watermark"><Image src="/hadeel-platform-logo.png" alt="منصة هديل" width={72} height={72} /></div></div><div className="pdf-actions"><a className="primary-button" href={selectedWork.file} download>تنزيل الملف <ArrowDown size={16} /></a><p className="pdf-note">يمكنك معاينة الملف أو تنزيله. الشعار ظاهر كعلامة مائية أثناء المعاينة.</p></div></section></div>}
