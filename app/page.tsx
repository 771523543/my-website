...
export default function Page() {
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      {selectedWork && <div className="pdf-modal-backdrop" role="presentation" onClick={() => setSelectedWork(null)}><section className="pdf-modal" role="dialog" aria-modal="true" aria-labelledby="pdf-title" onClick={(event) => event.stopPropagation()}><div className="pdf-modal-header"><h2 id="pdf-title">{selectedWork.title}</h2><button onClick={() => setSelectedWork(null)} aria-label="إغلاق المعاينة"><X size={20} /></button></div><div className="pdf-viewer"><iframe src={`${selectedWork.file}#toolbar=0&navpanes=0&scrollbar=1`} title={`معاينة ${selectedWork.title}`} /></div><p className="pdf-note">هذه معاينة داخلية للملف. قد يتيح المتصفح خياراته الخاصة بحسب الجهاز المستخدم.</p></section></div>}
