...
const services = [
  { icon: BookOpen, image: '/images/service-research.png', title: 'إعداد البحوث والتقارير', text: 'نساعدك في إعداد وتنسيق الأبحاث العلمية والجامعية وفق أحدث المعايير الأكاديمية وضوابط APA، مع ضمان الخلو من الاستلال والخطأ اللغوي.' },
  { icon: Presentation, image: '/images/service-presentation.png', title: 'العروض التقديمية', text: 'نصمم لك عروض PowerPoint احترافية تبرز أفكارك بوضوح وجاذبية، وتمنحك حضورًا قويًا وثقة أكبر في مناقشات مشاريع التخرج.' },
  { icon: GraduationCap, image: '/images/service-assignments.png', title: 'التكليفات والواجبات', text: 'نتولى حل ومتابعة واجباتك وأنشطتك الجامعية بدقة والتزام تام بالمواعيد، لنخفف عنك الضغط ونوفر وقتك للأهم.' },
  { icon: UserRound, image: '/images/service-cv.png', title: 'السيرة الذاتية CV', text: 'نصيغ لك سيرة ذاتية عربية وإنجليزية احترافية ومتوافقة مع أنظمة ATS، لتفتح لك أبواب الفرص الوظيفية بأفضل صورة.' },
  { icon: Sparkles, image: '/images/service-invitations.png', title: 'الدعوات الإلكترونية', text: 'نبتكر لك بطاقات دعوة إلكترونية لحفلات التخرج والمناسبات بتصاميم عصرية أنيقة، تعكس ذوقك وتترك انطباعًا لا يُنسى.' },
  { icon: Headphones, image: '/images/service-followup.png', title: 'متابعة مواد البلاك بورد', text: 'ندير ونتابع مقرراتك ومحاضراتك واختباراتك على البلاك بورد طوال الترم، لتضمن عدم تفويت أي موعد أو تكليف مهم.' },
]

type ServiceDetail = { intro: string; sections: { title: string; items: string[] }[] }

const serviceDetails: Record<string, ServiceDetail> = {
  'إعداد البحوث والتقارير': {
    intro: 'حلول بحثية متكاملة تساعدك على تقديم عمل أكاديمي منظم وموثق.',
    sections: [
...
export default function Page() {
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="services" data-reveal className="section container reveal-section"><div className="section-heading"><div><span className="section-kicker">خدماتنا</span><h2>حلول أكاديمية <em>باحترافية</em></h2></div><a className="text-button" href={whatsapp} target="_blank" rel="noreferrer">اطلب الآن <ArrowLeft size={17} /></a></div><div className="service-grid">{services.map(({ icon: Icon, image, title, text }) => <article data-reveal className="service-card reveal-section" key={title}><div className="service-image"><Image src={image} alt={title} fill sizes="(max-width: 800px) 100vw, 30vw" /></div><span className="service-icon"><Icon size={24} /></span><h3>{title}</h3><p>{text}</p><div className="service-actions"><a className="service-order" href={whatsapp} target="_blank" rel="noreferrer"><MessageCircle size={15} /> اطلب الخدمة</a><button className="service-browse" onClick={() => setSelectedService(title)}>تصفح التفاصيل <ChevronLeft size={15} /></button></div></article>)}</div></section>
