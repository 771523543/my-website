...
export default function Page() {
  ...
  const handleServiceClick = (title: string) => {
    if (title === 'إعداد البحوث والتقارير') {
      setSelectedService(title)
    }
  }

  const handleServiceKeyDown = (event: React.KeyboardEvent, title: string) => {
    if (title === 'إعداد البحوث والتقارير' && (event.key === 'Enter' || event.key === ' ')) {
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-background text-foreground">
      <div className="announcement">
        <Sparkles size={15} /> خصم خاص على خدمات منصة هديل لفترة محدودة <ArrowLeft size={15} />
      </div>

      <header className="site-header">
        <div className="container nav-wrap">
          <a className="brand" href="#top">
            <img className="brand-logo" src="/hadeel-platform-logo.svg" alt="شعار منصة هديل للخدمات الطلابية" width={54} height={54} />
          ...
          <nav className={menuOpen ? 'nav-links mobile-open' : 'nav-links'}>
            ...
            <a href="#values" onClick={() => setMenuOpen(false)}>قيمنا</a>
            <a href="#services" onClick={() => setMenuOpen(false)}>خدماتنا</a>
            <a href="#why" onClick={() => setMenuOpen(false)}>لماذا نحن</a>
            <a href="#contact" onClick={() => setMenuOpen(false)}>اتصل بنا</a>
          </nav>
          <div className="nav-actions">
            <a className="primary-button header-order" href={whatsapp} target="_blank" rel="noreferrer">
              <MessageCircle size={17} /> اطلب خدمتك الآن
            </a>
        ...
        <div className="hero-copy">
          ...
          <div className="hero-blue-card">
            ...
            <div className="path-line">
              <span className="path-dot active" /><span /><span className="path-dot active" /><span /><span className="path-dot active" />
            </div>
      ...
      <section id="services" data-reveal className="section container reveal-section">
        <div className="section-heading">
          <div>
            ...
            <h2>حلول أكاديمية <em>باحترافية</em></h2>
          </div>
          <a className="text-button" href={whatsapp} target="_blank" rel="noreferrer">اطلب الآن <ArrowLeft size={17} /></a>
        </div>
        <div className="service-grid">
          {services.map(({ icon: Icon, image, title, text }) => {
            const isClickable = title === 'إعداد البحوث والتقارير'
            return (
              <article
                data-reveal
                className={`service-card reveal-section ${isClickable ? 'service-card-clickable' : ''}`}
                key={title}
                onClick={() => handleServiceClick(title)}
                onKeyDown={(e) => handleServiceKeyDown(e, title)}
                role={isClickable ? 'button' : undefined}
                tabIndex={isClickable ? 0 : undefined}
              >
                <div className="service-image">
                  <Image src={image} alt={title} fill sizes="(max-width: 800px) 100vw, 30vw" />
                </div>
                <span className="service-icon"><Icon size={24} /></span>
                <h3>{title}</h3>
                <p>{text}</p>
                <a href={whatsapp} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}>
                  اطلب الخدمة <ChevronLeft size={16} />
                </a>
                {isClickable && <span className="service-more">عرض التفاصيل <ChevronLeft size={14} /></span>}
              </article>
            )
          })}
        </div>
      ...
      {selectedService === 'إعداد البحوث والتقارير' && (
        <div className="service-modal-backdrop" role="presentation" onClick={() => setSelectedService(null)}>
          <section className="service-modal" role="dialog" aria-modal="true" aria-labelledby="research-details-title" onClick={(e) => e.stopPropagation()}>
            <button className="service-modal-close" onClick={() => setSelectedService(null)} aria-label="إغلاق التفاصيل">
              <X size={20} />
            </button>
            <span className="section-kicker">تفاصيل الخدمة</span>
            <h2 id="research-details-title">إعداد البحوث والتقارير</h2>
            <p className="service-modal-intro">حلول بحثية متكاملة تساعدك على تقديم عمل أكاديمي منظم وموثق.</p>
            <div className="research-details">
              {researchDetails.map((detail) => (
                <div className="research-detail" key={detail.title}>
                  <h3>{detail.title}</h3>
                  <ul>
            ...
            <a className="primary-button" href={whatsapp} target="_blank" rel="noreferrer">
              اطلب هذه الخدمة <MessageCircle size={17} />
            </a>
          </section>
        </div>
      )}
      ...
      {selectedWork && (
        <div className="pdf-modal-backdrop" role="presentation" onClick={() => setSelectedWork(null)}>
          <section className="pdf-modal" role="dialog" aria-modal="true" aria-labelledby="pdf-title" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pdf-viewer">
              <iframe src={selectedWork.preview} title={`معاينة ${selectedWork.title}`} />
            </div>
          </section>
        </div>
      )}
      ...
      <section className="achievements-section container">
        ...
        >
          ...
          <button
            ...
            onClick={() => setAchievementIndex((achievementIndex + 1) % achievementImages.length)}
          ...
          <button
            ...
            onClick={() => setAchievementIndex((achievementIndex - 1 + achievementImages.length) % achievementImages.length)}
          ...
          <div className="achievement-dots">
            {achievementImages.map((image, index) => (
              <button
                key={image}
                className={index === achievementIndex ? 'active' : ''}
      ...
      <section id="why" className="why-section">
        <div className="container why-inner">
          ...
          <div className="feature-list">
            <div>
              <Check />
              <span><strong>سرعة فائقة في الإنجاز</strong><small>تنفيذ وتسليم في وقت قياسي.</small></span>
            </div>
            <div>
              <Check />
              <span><strong>جودة أكاديمية عالية</strong><small>مراجعة تدقيقية متكاملة لجميع الأعمال.</small></span>
            </div>
            <div>
              <Check />
              <span><strong>دعم ومتابعة مستمرة</strong><small>تواصل وتعديل حتى اعتماد العمل نهائيًا.</small></span>
            </div>
          </div>
        </div>
      </section>
