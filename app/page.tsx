...
export default function Page() {
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="story" className="section story-section container"><div className="story-visual"><div className="story-card"><BookOpen size={42} /><span>معرفة<br />تُنجز</span></div><div className="story-badge">منذ 2018</div></div><div className="story-copy"><span className="section-kicker">قصتنا</span><h2>بدأنا من إيماننا بأن<br /><em>كل طالب يستحق الدعم</em></h2><p>انطلقت منصة هديل لتكون الوجهة الموثوقة للطلاب والباحثين، وتحوّل التحديات الأكاديمية إلى خطوات واضحة قابلة للإنجاز. نعمل بشغف لنقدم حلولًا احترافية تراعي احتياجك وتساعدك على إكمال رحلتك بأعلى درجات الجودة.</p><a className="text-button" href={whatsappUrl} target="_blank" rel="noreferrer">تعرّف على هديل <ArrowLeft size={17} /></a></div></section>
      ...
      {/* قسم الخدمات التفاعلي المتحرك تلقائياً - تم تكبير البطاقات */}
      <section id="services" data-reveal className="section container reveal-section">
        ...
        <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '2rem', flexWrap: 'wrap', justifyContent: 'center' }}>
          <button onClick={() => setActiveCategory('all')} style={{ padding: '0.6rem 1.4rem', borderRadius: '25px', border: '1px solid #10b981', background: activeCategory === 'all' ? '#10b981' : 'transparent', color: activeCategory === 'all' ? '#fff' : 'inherit', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.95rem' }}>الكل</button>
          <button onClick={() => setActiveCategory('research')} style={{ padding: '0.6rem 1.4rem', borderRadius: '25px', border: '1px solid #10b981', background: activeCategory === 'research' ? '#10b981' : 'transparent', color: activeCategory === 'research' ? '#fff' : 'inherit', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.95rem' }}>بحوث وتقارير</button>
          <button onClick={() => setActiveCategory('design')} style={{ padding: '0.6rem 1.4rem', borderRadius: '25px', border: '1px solid #10b981', background: activeCategory === 'design' ? '#10b981' : 'transparent', color: activeCategory === 'design' ? '#fff' : 'inherit', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.95rem' }}>تصاميم وعروض</button>
          <button onClick={() => setActiveCategory('academic')} style={{ padding: '0.6rem 1.4rem', borderRadius: '25px', border: '1px solid #10b981', background: activeCategory === 'academic' ? '#10b981' : 'transparent', color: activeCategory === 'academic' ? '#fff' : 'inherit', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.95rem' }}>خدمات ومتابعة</button>
        </div>

        {/* حاوية الحركة التلقائية للخدمات */}
        <div className="services-marquee-container" style={{ overflow: 'hidden', padding: '1.2rem 0' }}>
              ...
              return (
                ...
                >
                  <button 
                    ...
                    style={{ position: 'absolute', top: '15px', left: '15px', zIndex: 10, background: 'rgba(255,255,255,0.95)', border: 'none', borderRadius: '50%', width: '38px', height: '38px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 3px 8px rgba(0,0,0,0.12)' }}
                  >
                    <Share2 size={18} style={{ color: '#333' }} />
                  </button>
                  <div className="service-image" style={{ height: '240px', position: 'relative' }}>
                    <Image src={service.image} alt={service.title} fill sizes="420px" style={{ objectFit: 'cover' }} />
                  </div>
                  <div className="service-content" style={{ padding: '1.8rem 1.5rem 1.5rem', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{ textAlign: 'center', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                      <span className="service-icon" style={{ display: 'inline-flex', justifyContent: 'center', alignItems: 'center', marginBottom: '0.6rem', width: '52px', height: '52px' }}>
                        <Icon size={28} />
                      </span>
                      <h3 style={{ fontSize: '1.4rem', marginTop: '0.3rem', fontWeight: 'bold', textAlign: 'center', width: '100%' }}>{service.title}</h3>
                      <p style={{ margin: '0.8rem 0 1.4rem', fontSize: '1rem', color: '#374151', lineHeight: '1.6', fontWeight: '500', textAlign: 'center', direction: 'rtl', height: '3.6rem', overflow: 'hidden', width: '100%' }}>
                        {service.shortText}
                      </p>
                    </div>
                    <button 
                      className="primary-button" 
                      style={{ width: '100%', justifyContent: 'center', fontSize: '1rem', padding: '0.8rem 1.2rem', borderRadius: '10px' }}
        ...
        {selectedDetailService && (
          <div 
            ...
            style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, padding: '1rem' }}
          >
            <section 
              ...
              style={{ backgroundColor: '#fff', borderRadius: '20px', padding: '2rem', width: '100%', maxWidth: '700px', maxHeight: '88vh', overflowY: 'auto', position: 'relative', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', direction: 'rtl' }}
            >
              <button 
                className="service-modal-close" 
                onClick={() => setSelectedDetailService(null)} 
                aria-label="إغلاق"
                style={{ position: 'absolute', top: '1.2rem', left: '1.2rem', background: '#f3f4f6', border: 'none', borderRadius: '50%', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
              >
                <X size={20} />
              </button>

              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '1.2rem', textAlign: 'center' }}>
                {selectedDetailService.icon && <selectedDetailService.icon size={36} style={{ color: '#10b981' }} />}
                <h2 style={{ fontSize: '1.45rem', fontWeight: 'bold', textAlign: 'center' }}>{selectedDetailService.title}</h2>
              </div>

              <div style={{ marginBottom: '1.4rem', background: '#f9fafb', padding: '1rem 1.2rem', borderRadius: '12px', textAlign: 'center' }}>
                <h3 style={{ fontSize: '1.1rem', marginBottom: '0.5rem', color: '#111', fontWeight: 'bold' }}>📌 عن الخدمة</h3>
                <p style={{ fontSize: '0.95rem', lineHeight: '1.7', color: '#444' }}>{selectedDetailService.about}</p>
              </div>

              {selectedDetailService.requirements && (
                <div style={{ marginBottom: '1.4rem' }}>
                  <h3 style={{ fontSize: '1.1rem', marginBottom: '0.5rem', color: '#111', fontWeight: 'bold', textAlign: 'center' }}>📋 ماذا نحتاج منك لطلب الخدمة؟</h3>
                  <ul style={{ listStyleType: 'none', paddingRight: 0, color: '#444', fontSize: '0.92rem', lineHeight: '1.7', textAlign: 'center' }}>
                    {selectedDetailService.requirements.map((req, idx) => <li key={idx} style={{ marginBottom: '0.3rem' }}>• {req}</li>)}
                  </ul>
                </div>
              )}
              ...
              {selectedDetailService.faqs && selectedDetailService.faqs.length > 0 && (
                <div style={{ marginBottom: '1.4rem' }}>
                  <h3 style={{ fontSize: '1.1rem', marginBottom: '0.5rem', color: '#111', fontWeight: 'bold', textAlign: 'center' }}>❓ الأسئلة الشائعة للخدمة</h3>
                  {selectedDetailService.faqs.map((faq, idx) => (
                    <div key={idx} style={{ marginBottom: '0.8rem', padding: '0.8rem 1rem', background: '#f8fafc', borderRadius: '8px', textAlign: 'center' }}>
                      <strong style={{ display: 'block', fontSize: '0.92rem', marginBottom: '0.3rem' }}>{faq.q}</strong>
                      <span style={{ fontSize: '0.88rem', color: '#555' }}>{faq.a}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div>
                <h3 style={{ fontSize: '1.15rem', marginBottom: '1rem', color: '#111', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontWeight: 'bold' }}>
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSendToWhatsapp(); }} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '0.3rem', fontWeight: 'bold', textAlign: 'right' }}>اسم الطالب / الطالبة *</label>
                    <input type="text" required placeholder="أدخل اسمك الكامـل" value={formData.studentName} onChange={(e) => setFormData({ ...formData, studentName: e.target.value })} style={{ width: '100%', padding: '0.6rem 0.85rem', borderRadius: '8px', border: '1px solid #ccc', fontSize: '0.92rem' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '0.3rem', fontWeight: 'bold', textAlign: 'right' }}>الرقم الجامعي</label>
                    <input type="text" placeholder="أدخل الرقم الجامعي (اختياري)" value={formData.universityId} onChange={(e) => setFormData({ ...formData, universityId: e.target.value })} style={{ width: '100%', padding: '0.6rem 0.85rem', borderRadius: '8px', border: '1px solid #ccc', fontSize: '0.92rem' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '0.3rem', fontWeight: 'bold', textAlign: 'right' }}>ملف المتطلبات أو اسم الملف</label>
                    <input type="text" placeholder="أدخل عنوان الملف أو رابط جوجل درايف" value={formData.fileName} onChange={(e) => setFormData({ ...formData, fileName: e.target.value })} style={{ width: '100%', padding: '0.6rem 0.85rem', borderRadius: '8px', border: '1px solid #ccc', fontSize: '0.92rem' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '0.3rem', fontWeight: 'bold', textAlign: 'right' }}>ملاحظات وإرشادات إضافية</label>
                    <textarea rows={3} placeholder="أدخل أي شروط خاصة أو موعد التسليم..." value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} style={{ width: '100%', padding: '0.6rem 0.85rem', borderRadius: '8px', border: '1px solid #ccc', fontSize: '0.92rem' }} />
                  </div>
                  <button type="submit" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: '#25D366', color: '#fff', border: 'none', padding: '0.85rem', borderRadius: '10px', fontWeight: 'bold', fontSize: '1rem', cursor: 'pointer', marginTop: '0.5rem' }}>
                    تأكيد وإرسال الطلب عبر الواتساب <MessageCircle size={20} />
                  </button>
                </form>
              </div>
            </section>
